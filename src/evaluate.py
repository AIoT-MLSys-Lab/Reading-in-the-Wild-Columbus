...
def main(cfg: DictConfig):
    ...
    # Do not change ActivityType; leave it as in the metadata file.
    ...
    all_data = []
    # We assume that metadata has columns: ActivityType, Medium, ShortTextOrPara, csv_file, ContainsNonText, Platform, ExtraTags, and filename.
    roc_data = metadata_valid[
        ['ActivityType', 'Medium', 'ShortTextOrPara', 'csv_file', 'ContainsNonText', 'Platform', 'ExtraTags']]

    # Define a helper to convert ActivityType to a binary label.
    def activity_to_binary(x):
        # For this example, treat 'reading_positive' and 'reading_searching' as reading (1),
        # and others (e.g. 'negative', 'reading_negative', 'reading_hard_negative') as not reading (0)
    ...
    for _, row in tqdm(roc_data.iterrows()):
        cat = row['binary_act']
        fname = row['csv_file']
        medium = row['Medium']
    ...
    for col in cfg.modalities:
        if col not in df_all.columns:
            ...
            continue
        # Get a simple string representation for display (using our hard-coded modality map)
        modality_str = map_bool_to_str(MODALITY_MAP.get(col, {}))
        df_num = df_all[['act', col]].dropna()
        # Compute metrics. Only keep the metrics specified in the config.
